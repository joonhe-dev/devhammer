...
const TEMP_DIR = join(process.cwd(), '.tmp-fs-test-external');
...
describe('fs utilities', () => {
  ...
  describe('readFileContent', () => {
    ...
    it('reads UTF-8 content correctly', async () => {
      const filePath = join(TEMP_DIR, 'utf8.txt');
  ...
  describe('writeJson', () => {
    ...
    it('handles null and undefined values', async () => {
      const filePath = join(TEMP_DIR, 'nullable.json');
      const data = { a: null, b: 0, c: '' };
      await writeJson(filePath, data);
      const result = await readJson<typeof data>(filePath);
